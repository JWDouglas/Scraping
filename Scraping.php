...
/* Created by: Lara Vieira - https://jwdouglas.net
...
class Scraping {
    ...
    private static string $cacheFolder = SCRAPING_CACHE;
    private string $session;

    public static function cache(string $name, mixed $cache=null, string $expire=null) {
        $expire = isset($expire)?$expire:'P1D'; // Expire in 1 day
        ...
        if(isset($cache))
            file_put_contents($file, json_encode(array(
                'total' => count($cache),
                'updated' => (new DateTime('now'))->format(DateTime::RSS),
                'expire' => (new DateTime('now'))->add(new DateInterval($expire))->format(DateTime::RSS),
                $name => $cache,
            )));
    ...
    public static function cacheFolder(string $folder=null) {
        return self::$cacheFolder = isset($folder)?$folder:self::$cacheFolder;
    }
    ...
    public function __construct(
        ...
        if($useSession && session_status() != PHP_SESSION_ACTIVE)
            session_start();
        self::$cacheFolder = SCRAPING_CACHE;
    }

    public function useSession(bool $useSession=null) {
        return $this->useSession = isset($useSession)?$useSession:$this->useSession;
    }
    
    public function userAgent(string $userAgent=null) {
        return $this->userAgent = isset($userAgent)?$userAgent:$this->userAgent;
    }
    
    public function server(string $server=null) {
        return $this->server = isset($server)?$server:$this->server;
    }
    
    public function session(string $session=null) {
        return $this->session = isset($session)?$session:$this?->session;
    }
    
    private function hasSession() {
        return $this->useSession?isset($this->session):false;
    }
    
    private function sessionName(string $sessionName=null) {
        return $this->sessionName = isset($sessionName)?$sessionName:$this->sessionName;
    }

    public function get(string $uri, bool $headers=false) {
    ...
    public function post(string $uri, array $data, array $header=null, bool $headers=false) {
    ...
    private function proccess(array $data, bool $headers) {
        if(!$headers)
            return $data;
